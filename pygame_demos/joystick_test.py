...
if not joysticks:
    print(f"Sorry, no joystick to test")
    quit_game()
...
def draw_axis(surface, x, y, axis_x, axis_y, size):
    ...
    draw_x = int(x + (axis_x * size + size) / 2.0)
    draw_y = int(y + (axis_y * size + size) / 2.0)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if pygame.K_0 <= event.key <= pygame.K_1:
    ...
    axis_size = min(256, SCR_W / (joystick.get_numaxes()/2))
    pygame.draw.rect(screen, (255, 255, 255), (0, 0, SCR_W, SCR_H))
    ...
    x, y = 0.0, 390.0
    button_width = SCR_W / joystick.get_numbuttons()
